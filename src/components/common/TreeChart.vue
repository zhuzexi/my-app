			...
			<td 
				...
				<div :class="{ node: true}">
					...
					<div class="person" @click="$emit('click-node', treeData)">
						<div class="avat"><img :src="treeData.image_url" /></div>
						<div class="name">{{treeData.memberName}}</div>
					</div>
					
					<!-- <div class="person" v-if="treeData.mate" @click="$emit('click-node', treeData.mate)">
						<div class="avat"><img :src="treeData.mate.image_url" /></div>
						<div class="name">{{ treeData.mate.name }}</div>
					</div> -->
					
				</div>
				
				<div class="extend_handle" v-if="treeData.children" @click="toggleExtend(treeData)">
	...
	<!-- <table> -->
		<tr v-if="treeData.children && treeData.extend">
			...
			<div 
				...
				:key="index" 
				colspan="2" 
				:class="{'peiou': children.role == '配偶'}"
				:style="{'right': children.role == '配偶' ? (treeData.children.length < 5 ? '-' : '') + (72 / (treeData.children.length - 1 || 1)) + 'px' : 'unset'}"
				v-if="children.role == '配偶'"
			>
				<TreeChart :json="children" @click-node="$emit('click-node', $event)" />
			</div>
...
export default {
	...
	mounted() {
		// console.log(this.treeData.memberName, '<<<<<..........根名称'); //根名称
	},
...
.childLevel:first-of-type:after {
	left: 50%;
	height: 15px;
	border: 2px solid;
	border-color: #ccc transparent transparent #ccc;
...
.childLevel:last-of-type:before,.childLevel:first-of-type:before{
	display: none;
}
