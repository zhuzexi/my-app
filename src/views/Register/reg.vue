<template>
	<div>
		...
		<section class="page">
			<div class="register-page">
				<div class="register-wrap">
					<div class="register-text">
						<input @mouseenter="focusText" @mouseleave="blurText" v-model="phone" type="tel" autofocus="autofocus" class="register-phone" placeholder="请输入手机号" />
					...
					<div class="register-text">
						<input @mouseenter="focusText" @mouseleave="blurText" v-model="invitation" type="text" class="register-invitation" placeholder="填写邀请码 (非必填)" />
						<i class="iconfont icon-close" data-close="invitation" v-show="!invitationClose" @click="clearText"></i>
					</div>
				</div>
			</div>
			...
			<button class="register-button" @click="register" :class="{ active: removeSpace(phone) && removeSpace(captcha) && removeSpace(password) && removeSpace(password2) }">
				完 成 注 册
			</button>
...
export default {
	...
	data() {
		return {
			headbg:require('../../assets/register_bg.png'),
			phone: '',
			password: '',
			password2: '',
			captcha: '',
	...
	methods: {
		...
		async code() {
			...
			//拼接参数
			//接口调用
//			http://api.hejiawh.com/member-server/vmall/memberInfoRegister/authCode?phone=18984896415
			this.$axios.post('member-server/vmall/memberInfoRegister/authCode?phone=' + this.phone,{}, {
		...
		async register() {
			...
			//拼接参数
			let params = '?phone=' + this.phone + '&password=' + this.password + '&password2=' + this.password2 + '&recommendPhone=' + this.invitation + '&code=' + this.captcha;

			//接口调用
			this.$axios.post('/member-server/vmall/memberInfoRegister/register' + params, {}).then(res => {
				// console.log(res,'<-----info');
				if (res.data.status == '200') {
					this.$toast('恭喜您，注册成功！');
