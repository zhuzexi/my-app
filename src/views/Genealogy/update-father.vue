<template>
	<div class="formbg">
		<m-header :mTitle="'修改族人信息'"></m-header>
		...
		<van-form @submit="saveUserInfo">
			...
			<van-field required v-model="surnames" name="姓氏" label="姓氏" placeholder="请填写您的姓氏" />

			<van-field required v-model="memberName" name="姓名" label="姓名" placeholder="请填写您的姓名" />

			<van-field required readonly clickable name="datetimePicker" :value="timeInputValue" label="出生日期" placeholder="请选择出生日期" @click="showDatePopup" />
			...
			<van-field required v-model="detailedAddress" name="详细地址" label="详细地址" />
			...
			<van-button round type="info" native-type="submit" style="width: 75%; margin: 20px auto;">保存修改</van-button>
		</van-form>
	</div>
</template>
...
　　　		var formData = new FormData(); //构造一个 FormData，把后台需要发送的参数添加
		...
		　　this.$axios.post('/file-server/file/multiUploadImg', formData, {
			...
			.then(res => {//将formDate文件上传到阿里云服务器，会返回图片地址
				if(res.data.status == 200){
					this.bgList = [{url: 'http://api1.hejiawh.com/' + res.data.data[0]}]
		...
		async selectPeopleInfo() {
			...
			this.$axios
				...
				.then(res => {
					...
					if(memberInfo.memberHeadPortrait && memberInfo.memberHeadPortrait.length != 0){ //头像
						this.fileList = [{url: memberInfo.memberHeadPortrait}];
					}
					if(memberInfo.backgroundChart && memberInfo.backgroundChart.length != 0){ //背景
						this.fileList = [{url: memberInfo.backgroundChart}];
					}
		...
		async saveUserInfo() {
			...
			this.$axios.post('/member-server/vmall/memberRepairSpectrum/upRepairSpectrum' ,this.params, {
					headers: {
						'content-type': 'application/json',
						memberAccessToken: this.memberKey.memberAccessToken
					}
